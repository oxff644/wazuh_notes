...
​
    ...
    if "labels" in alert["agent"]:
        agent_labels = alert["agent"]["labels"]
        if "integration" in agent_labels:
            if agent_labels["integration"] == "virustotal":
                skip_agent = False
​
    if skip_agent:
       agent_id = alert["agent"]["id"]
       debug(f"Agent {agent_id} skipped from Virus Total request")
       return(0)
​
    # Create alert
    alert_output["virustotal"] = {}
