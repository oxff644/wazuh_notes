...
while true; do
    read -p "Do you wish to configure Wazuh with this ruleset? WARNING - This script will replace all of your current custom Wazuh Rules. Please proceed with caution and it is recommended to manually back up your rules.....continue?" yn
    case $yn in
        [Yy]* ) break;;
        [Nn]* ) exit;;
...
cloneRules() {
    ...
    if [ ${sys_type} == "yum" ]; then
        ...
        logger -e "Backing up current rules into /tmp/wazuh_rules_backup/"
        \cp -r /var/ossec/etc/rules/* /tmp/wazuh_rules_backup/
        git clone https://github.com/oxff644/Wazuh_notes.git
        cd /tmp/Wazuh-Rules
        find . -name '*xml' -exec mv {} /var/ossec/etc/rules/ \;
        find /var/ossec/etc/rules/ -name 'decoder-linux-sysmon.xml' -exec mv {} /var/ossec/etc/decoders/ \;
        find /var/ossec/etc/rules/ -name 'yara_decoders.xml' -exec mv {} /var/ossec/etc/decoders/ \;
